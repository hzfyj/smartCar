...
void IMU_IIC_Init()
{
	...
	gpio_init(C10,GPO,1);//k60移植到k66的引脚初始化
	
//  GPIO_InitTypeDef GPIO_InitStruct1;//
//	GPIO_InitStruct1.GPIO_Pin = IIC_SCL_PIN;                 
//	GPIO_InitStruct1.GPIO_InitState = Bit_SET;//
//	GPIO_InitStruct1.GPIO_IRQMode = GPIO_IT_DISABLE;//改
//	GPIO_InitStruct1.GPIO_Mode = GPIO_Mode_OPP;//改
...
{
	IIC_start();		//启动
	send_byte( (slaveaddress<<1) | 0x00);	//写入设备ID及写信号
...
uint8_t read_reg(uint8_t slaveaddress, uint8_t address)
{
	...
	send_byte(address);	//X地址
	IIC_start();		//重新发送开始
	send_byte((slaveaddress<<1) | 0x01);	//写入设备ID及读信
	ret = read_byte();	//读取一字节
	IIC_stop();
	return ret;
}
